...
const ProjectCard = ({ project, index }) => {
    return (
        <div
            key={index}
            className={`text-white flex flex-col md:flex-row items-start ${index % 2 === 0 ? '' : 'md:flex-row-reverse'} fade-up-projects`}
        >
            <div className="w-full md:w-1/2 p-4">
                <a href={project.link} target="_blank" rel="noopener noreferrer">
                    <img
                        src={project.imgSrc}
                        alt={project.title}
                        className="w-full h-auto rounded-md border-[2.5px] border-[#163849] duration-300 ease-in-out shadow-2xl hover:scale-[1.025] hover:shadow-lg"
                    />
                </a>
            </div>
